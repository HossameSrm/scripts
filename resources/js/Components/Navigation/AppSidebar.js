        ...
        },
        {
          ...
          entries: [permitted(adminRoute) ? {
            ...
            children: [
              ...
              permitted(historyRoute)
                ? { ...historyRoute, key: null, label: 'Journal d’activité', href: 'history.html?scope=all', query: true }
                : null
            ].filter(Boolean)
          } : null].filter(Boolean)
        },
      ...
      return `
        <div class="app-brand-block">
          <a href="dashboard.html" class="app-brand-link">
            ...
            <span class="app-brand-copy min-w-0">
              <strong class="app-brand-name">SRM Workspace</strong>
              <small class="app-brand-subtitle">Grands Comptes</small>
            </span>
          </a>
          <button id="sidebarCollapse" class="app-sidebar-collapse" type="button" aria-label="Réduire le menu">
            ${window.UI.icon('chevron')}
          </button>
        </div>
        ...
        <div class="app-profile-card">
          <div class="app-profile-main">
            ...
            <div class="app-profile-copy min-w-0 flex-1">
              <p class="truncate app-profile-name">${window.UI.esc(user.name)}</p>
              <p class="truncate app-profile-role">${roleLabel} · ${window.UI.esc(user.matricule)}</p>
            </div>
            ${user.is_owner ? '<span class="owner-crown" title="Propriétaire">★</span>' : ''}
          </div>
          <button id="sidebarLogout" class="app-logout-button" type="button">
            ${window.UI.icon('logout')}<span>Se déconnecter</span>
          </button>

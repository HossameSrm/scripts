(function (window) {
  ...
  class NavigationView {
    ...
    initials(name) { return String(name || 'U').trim().split(/\s+/).slice(0, 2).map(x => x[0]).join('').toUpperCase(); }
    ...
    renderSidebar({ db, user, permissions, page }) {
      ...
      return `
        ...
        <div class="app-session-card mt-auto p-4">
          <div class="flex items-center gap-3"><div class="ui-avatar">${this.initials(user.name)}</div><div class="min-w-0"><p class="truncate font-black text-slate-900">${window.UI.esc(user.name)}</p><p class="truncate text-xs font-semibold text-slate-500">${user.role === 'admin' ? 'Administrateur' : 'Utilisateur'} · ${window.UI.esc(user.matricule)}</p></div></div>
      ...
      return `
        <div class="flex h-full items-center justify-between gap-3">
          ...
          <div class="flex items-center gap-2 sm:gap-3">
            ...
            <div class="hidden items-center gap-2 rounded-2xl border border-white/90 bg-white/75 px-3 py-2 shadow-sm sm:flex"><div class="ui-avatar !h-9 !w-9 !rounded-xl text-xs">${this.initials(user.name)}</div><div class="max-w-[150px]"><p class="truncate text-sm font-black text-slate-800">${window.UI.esc(user.name)}</p><p class="truncate text-[11px] font-semibold text-slate-500">${user.role === 'admin' ? 'Administrateur' : 'Utilisateur'}</p></div></div>
          </div>
        </div>`;
    }
  }

  ...
  class DashboardView {
    render(data, permissions) {
      ...
      document.getElementById('statsGrid').innerHTML = [
        { label: 'Utilisateurs', value: stats.users ?? 0, icon: 'users', tone: 'blue', note: `${stats.activeUsers ?? 0} actif(s)` },
        { label: 'Clients', value: stats.clients ?? 0, icon: 'client', tone: 'cyan', note: 'Base clients active' },
        { label: 'Contrats', value: stats.contracts ?? 0, icon: 'contract', tone: 'violet', note: 'Tous services confondus' },
      ...
      document.getElementById('quickActions').innerHTML = actions.length ? actions.map(item => `<a href="${item.href}" class="quick-action"><div class="quick-action-icon">${window.UI.icon(item.icon)}</div><div><p class="quick-action-title">${item.title}</p><p class="quick-action-text">${item.text}</p></div></a>`).join('') : window.UI.emptyState('Aucun module disponible', 'Contactez votre administrateur pour obtenir un accès.');

      const recent = data.recent || [];
      document.getElementById('recentActivity').innerHTML = recent.length ? recent.map(item => `<div class="ui-list-item"><div class="ui-list-icon">${window.UI.icon(item.action === 'login' ? 'user' : 'history')}</div><div class="min-w-0 flex-1"><p class="ui-list-title">${window.UI.esc(item.description)}</p><p class="ui-list-text">${window.UI.esc(item.user_name)} · ${window.UI.formatDate(item.created_at)}</p></div>${window.UI.badge(item.module || 'système', 'slate')}</div>`).join('') : window.UI.emptyState('Aucune activité', 'Les activités récentes apparaîtront ici.', 'history');
      document.getElementById('dashboardRoot')?.classList.remove('opacity-0');
    }
    renderError(message) { document.getElementById('recentActivity').innerHTML = window.UI.emptyState('Erreur de chargement', message, 'warning'); }
  }
